...
public class Authenticate {
    ...
    String secretString, secureCode = "mysecuresecurecodeOnlyeternitycanbringusclosertotheHeavenlyPrinciplesIamnolongertheshadowMineisthemostsupremeandnobleformOnlyeternitycanbringusclosertotheHeavenlyPrinciplesIamnolongertheshadowMineisthemos";
 
    public Authenticate() {
 
        signatureAlgorithm = SignatureAlgorithm.HS256;
        
        secretString = Encoders.BASE64.encode(secureCode.getBytes());
    ...
    public Entry<Boolean, String> verify(String jwt) throws UnsupportedEncodingException {
        ...
        } catch (JwtException ex) {       // (5) PRINTS ERROR INTO CONSOLE ; if remove it will not bc error is caught (try/catch)
            ex.printStackTrace();
